...
// more info – https://indesignsecrets.com/easy-diacritics-and-other-tough-glyphs.php
...
function compose_character ()
  ...
  if (app.selection.length > 0 && app.selection[0].constructor.name == "InsertionPoint" && input_characters == "\\")
    ...
    switch (input_characters.length)
      ...
      case 2: combine (input_characters); break;
...
function type_char (ch) {enter_string (ch);}
...
function get_input ()
  ...
  //  var default_font = ScriptUI.newFont (ScriptUI.applicationFonts.palette.name, "Regular", 16);
    var inp = w.add ('group');
      var prmpt = inp.add ('statictext', undefined, 'Input:');
   //         prmpt.graphics.font = default_font;
      var response = inp.add ('edittext {text: "?=Help", characters: 7, active: true}');
  //    response.graphics.font = default_font;
    var b = w.add ('button', undefined, 'OK');
  if (w.show () == 2) exit();
  if (response.text == '?' || response.text == '?=Help')
    {
    show_help ();
    return get_input ()
    }
